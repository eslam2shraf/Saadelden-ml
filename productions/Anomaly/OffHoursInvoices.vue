  ...
  <BaseCard class="bg-L_D_Yellow-50 p-2">
    ...
    <template v-else>
      <BaseTable
        v-if="
          offHoursInvoices &&
          offHoursInvoices.data &&
          offHoursInvoices.data.length !== 0
        "
        class="responsive_table table_raduis_bottom"
        :data="offHoursInvoices.data"
                ...
                <div>
                  ...
                  >
                    ...
                    >
                      ...
                      <span> {{ item.quantity }} {{ $t("items") }} </span>
      ...
      <div class="card-footer m-auto w-80">
        <BasePagenation
          v-if="offHoursInvoices && offHoursInvoices.meta"
          :totalPages="
            Math.ceil(
              offHoursInvoices.meta.total / offHoursInvoices.meta.per_page
            )
          "
          :perPage="offHoursInvoices.meta.per_page"
          :currentPage="currentPage"
          :getData="paginatedInvoices"
        />
      </div>
    </template>
  </BaseCard>
...
export default {
  components: { BackToTopButton, CheveronRight, BaseModal },
  name: "OffHoursInvoices",
  data() {
    return {
      currentPage: 1,
      activeModal: 0,
      ...
      invoicesHead: [
        ...
        { id: 4, name: `${this.$t("price")}` },
        { id: 5, name: `${this.$t("creation_date")}` },
  ...
  created() {
    this.paginatedInvoices(this.currentPage);
  },
  methods: {
    paginatedInvoices(page) {
      this.isLoading = true;
      this.currentPage = page;
      this.$store
        .dispatch("productions/Anomaly/fetchOffHoursInvoices", {
          page: page,
          per_page: 15,
        })
        .then(() => {
