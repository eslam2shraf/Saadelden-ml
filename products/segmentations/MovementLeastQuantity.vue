...
export default {
  ...
  created() {
    this.$store
      .dispatch("products/segmentaion/fetchLessQuantityMovement")
      .then(() => {
        let len = 0;
        if (this.movementLessQuantity && this.movementLessQuantity.dates) {
          len = this.movementLessQuantity.dates.length;
        }
        ...
        if (this.movementLessQuantity) {
          labels = this.movementLessQuantity.dates;
          Object.entries(this.movementLessQuantity.data).forEach(
            ([key, value], j) => {
